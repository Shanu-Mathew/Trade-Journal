...
import { Edit2, Trash2, TrendingUp, TrendingDown, Eye, Copy, X, ArrowUp, ArrowDown } from 'lucide-react';
...
type SortField = 'symbol' | 'direction' | 'quantity' | 'entry_price' | 'exit_price' | 'profit_loss' | 'entry_timestamp' | null;
...
export default function TradesList({ trades, accounts, onEdit, onDelete }: TradesListProps) {
  ...
  const [activeColumn, setActiveColumn] = useState<SortField>(null);

  const tdRefs = useRef<Record<string, HTMLTableCellElement | null>>({});
  const closeTimers = useRef<Record<string, number | null>>({});
  const sortInactivityTimer = useRef<number | null>(null);

  const POPOVER_WIDTH = 320;
  const POPOVER_MARGIN = 8;
  const CLOSE_DELAY = 100;
  const ANIM_DURATION = 150;
  const SORT_FADE_DELAY = 2000;
  ...
  // FIXED handleSort: use functional updaters so toggles won't suffer from stale state
  const handleSort = (field: SortField) => {
    ...
    resetSortInactivityTimer();
  };

  const resetSortInactivityTimer = () => {
    if (sortInactivityTimer.current) {
      window.clearTimeout(sortInactivityTimer.current);
    }
    sortInactivityTimer.current = window.setTimeout(() => {
    ...
    return [...trades].sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'entry_timestamp':
          aVal = a.entry_timestamp ? new Date(a.entry_timestamp).getTime() : 0;
          bVal = b.entry_timestamp ? new Date(b.entry_timestamp).getTime() : 0;
          break;
        default:
          return 0;
      }
  ...
  // SortableHeader uses canonical sortField/sortDirection + hoveredColumn to decide arrow state.
  const SortableHeader = ({ field, children, align = 'left' }: { field: SortField; children: React.ReactNode; align?: 'left' | 'right' | 'center' }) => {
    const isActive = sortField === field;
    const showArrow = (hoveredColumn === field) || isActive;
    const arrowOpacityClass = isActive ? 'opacity-100' : 'opacity-70';

    return (
      <th
        className={`px-4 py-3 text-${align} text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider cursor-pointer select-none hover:bg-slate-100 dark:hover:bg-slate-700/30 transition-colors`}
      ...
      >
        <div className={`flex items-center gap-2 ${align === 'right' ? 'justify-end' : align === 'center' ? 'justify-center' : 'justify-start'}`}>
          <span>{children}</span>
          <span
            className={`transition-all duration-200 ${showArrow ? arrowOpacityClass : 'opacity-0'}`}
        ...
        <table className="w-full">
          <thead className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700">
            <tr>
              ...
              <SortableHeader field="entry_price" align="right">Entry</SortableHeader>
              <SortableHeader field="exit_price" align="right">Exit</SortableHeader>
              <SortableHeader field="profit_loss" align="right">P&L</SortableHeader>
              ...
              <SortableHeader field="entry_timestamp">Date</SortableHeader>
            ...
            {sortedTrades.map((trade) => {
              ...
              return (
                <tr key={trade.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                  ...
                  <td className="px-4 py-4 text-right text-sm text-slate-900 dark:text-white">${(trade.entry_price ?? 0).toFixed(2)}</td>

                  <td className="px-4 py-4 text-right text-sm text-slate-900 dark:text-white">{trade.exit_price !== null && trade.exit_price !== undefined ? `$${trade.exit_price.toFixed(2)}` : '-'}</td>
                  ...
                  <td className="px-4 py-4 text-right">
                    <span className={`font-semibold ${(trade.profit_loss ?? 0) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                      {formatCurrency(trade.profit_loss ?? null)}
                    </span>
                    {trade.profit_loss_percent !== null && trade.profit_loss_percent !== undefined && <div className="text-xs text-slate-500 dark:text-slate-400">{trade.profit_loss_percent.toFixed(2)}%</div>}
                  </td>
